const levels = [
	...
	{
		text:
			'39556 8793 36587 80348 6552 60005 4 280 665 7 9899 859 2777 0 0 2 2047 2910 71 3 3145 51 48871 9064 721 336229 3 84 3 434 0 60 470 6280 84592 9 36949 37 9259 46 720 436 288 80 53 52 7159 04 741 82663991 96 6 9 98 45 4 16681 90 44 2365 11 4 454 90 9482 58 390 684 161 774 3 81938 37908 63029 6174 16 306 86 1545 6 99 73 851 727 19453 84135 9564 42 89 54 4 43 8846 567 3 9 26 55 0 63 77 4846 765 447 93 3 6 886 15 7 7521 16685 5280 4262 44 839 2717 929 05 543 18525 59 4 8 1 26 7 56 2526 600 4 85099 21 770 0 6 074 251 68399018 417 789 39 93 181 18968 4 6919 41 9365 39939 9 554 161 2 75087 211 8 6140 9 4185 87792 82 450 371 971 496 21575 78 57014 34 4 96932 87 95 2 817 9 3 6325 3 261 831 64 5934 5 4445 647 501 0 95 772 2417',
		expects: ['04 741 82663991', '074 251 68399018']
	},
	{
		text:
			'22 fat scared gray lions 88 fat clever black tigers 92 fat cheeky white jaguars 2 thin fearless brown cougars 49 small playful brown lynx 80 thin shy brown jaguars 52 fat scared brown lynx 58 skinny lazy white cougars 33 slim cheeky red tigers 78 slim scared red jaguars 22 massive proud gray lynx 7 big scared gray tigers 7 slim cheeky brown jaguars 23 massive magical red tigers 9 massive lazy brown jaguars 97 massive silly black lynx 89 slim playful brown leopards 8 big cheeky brown cheetahs 21 short shy white cougars 85 thin timid red jaguars',
		expects: [
			'88 fat clever black tigers',
			'33 slim cheeky red tigers',
			'7 big scared gray tigers',
			'23 massive magical red tigers'
		]
	}
	// [4-7] dogs| 9 cats
...
class Game {
	constructor(levels) {
		...
		document.getElementById('next').onclick = () => this.goToLevel(this.levelPtr + 1);
	...
	refreshDisplay() {
		this.displayData(
			this.getHighlightedText(this.currentLevel.text, this.getRegExp(`(${this.pattern})`, this.options))
		);
	...
	displayMatches(data, rePattern) {
		if (rePattern) {
			this.dMatches.innerHTML = JSON.stringify(Array.from(data.match(rePattern) || []));
		} else {
			this.dMatches.innerHTML = '';
		}
	}
	...
	goToLevel(newLevel) {
		...
		if (this.currentLevel) {
			...
			document.querySelectorAll('.js-flag').forEach(dFlag => {
				dFlag.checked = false;
			});
			this.onPatternUpdated();
			this.dPattern.focus();
		} else {
			document.getElementById(`scene-end`).setAttribute('aria-hidden', false);
			document.querySelector('.challenge').setAttribute('aria-hidden', true);
			this.dSuccess.setAttribute('aria-hidden', true);
		}
	}
}
...
window.onload = () => {
	...
	window.debugLevel = game.goToLevel.bind(game);
};
